...
// Idempotent schema. Kept in sync with db/schema.sql (used by scripts/setup-db.mjs).
// Prices/mileage use `integer` (max ~2.1e9) so postgres.js returns JS numbers,
// not bigint strings; ample for ZAR prices and odometer values.
export const DDL = `
...
const globalForSchema = globalThis as unknown as { __mmSchemaReady?: Promise<void> };

/** Run the idempotent DDL exactly once per process. */
export function ensureSchema(sql: Sql): Promise<void> {
  if (!globalForSchema.__mmSchemaReady) {
    globalForSchema.__mmSchemaReady = sql.unsafe(DDL).then(
      () => {},
      (err) => {
        // Reset so a later request can retry (e.g. transient connect failure).
        globalForSchema.__mmSchemaReady = undefined;
        throw err;
      },
    );
  }
  return globalForSchema.__mmSchemaReady;
}
