...
- **STATUS: LIVE + persistent.** Vercel Postgres (Neon) is connected (`POSTGRES_URL` etc.), and `AUTH_SECRET` + `ADMIN_EMAIL` (`admin@mitmakmotors.co.za`) + `ADMIN_PASSWORD` + `ADMIN_NAME` are set on Production (all Sensitive/Encrypted). The DB auto-migrated + seeded 406 cars; create/read/delete verified against Postgres. Admin password is held by Jordan (NOT in this repo). Optional still-unset: Vercel Blob (`BLOB_READ_WRITE_TOKEN`, for file uploads; URL-paste works without it) + `RESEND_API_KEY` (enquiry/reset emails; saved+logged without it). Full steps in `ADMIN_SETUP.md`.
...
- **Inventory: ~396 live cars** in Postgres (seed `data/vehicles.json`, re-scraped from the live site) + a **private admin dashboard at `/admin`**. To refresh to the source site's current stock: re-run `scripts/scrape-inventory.mjs`, commit + deploy, then `POST /api/admin/vehicles/resync` (manager+, authed) — it replaces scrape-origin cars in the DB and keeps any hand-added ones. See the NEW section above.
